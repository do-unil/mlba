...
To obtain the optimal parameters, one of the best algorithm is the {\bf Newton-Raphson} algorithm. It requires
\begin{itemize}
...
\item The larger the coefficient, the larger the increase. However, the increase is not linear and depends on the other features.
...
\vspace{0.2cm}
Also, a model with a lot of predictions close to 0.5 can be considered as poor because of its uncertainty\footnote{A little thought on that will brings us back to the entropy.}, unless the data itself is inherently ambiguous.
\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\item The log-likelihood part, derived from a gaussian model, equals
$$
...
\item The log-likelihood part, derived from a Bernoulli model, equals
$$
