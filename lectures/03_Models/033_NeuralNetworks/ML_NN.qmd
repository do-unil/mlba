...
0.6 ![](../../Graphs/NN_Iris.pdf){width="8cm" fig-align="center"}
...
![](../../Graphs/Activation_graphs.pdf){width="10cm" fig-align="center"}
...
The most used algorithm is the **gradient descent**.
...
very heavy on a NN.\
**Backpropagation** is a method exploiting the iterative structure of
the network to compute this gradient.
...
$$(10+1)\times 16 + (16+1)\times 8 + (8+1)\times 3 = 339$$ weights.\
With such a large number of parameters, the model is at risk of
overfitting the training set by learning too much. One can regularize
...
Again there is no simple way to set the penalty parameters $\lambda_1$
and $\lambda_2$. Note that it is possible to have different penalty
parameters in different layers.\
Unlike regression and trees, and like SVM, this regularization can help
to avoid overfitting but cannot be interpreted easily.
