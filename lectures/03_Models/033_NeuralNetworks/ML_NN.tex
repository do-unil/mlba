...
\begin{eqnarray*}
\eta_{21} &=& \theta_{21}^{(0)} + \theta_{21}^{(1)} Z_{11} + \theta_{21}^{(2)} Z_{12} \\
 &=& -5.2\times 10^{-4} + 5.5\times 10^{-4} Z_{11} + 1.0 Z_{12}
\end{eqnarray*}
The calculation is repeated for each node giving $\eta_{21}, \eta_{22}, \eta_{23}$. Then, the value of the node is obtained applying the {\bf softmax function}:
\begin{eqnarray*}
Z_{21} &=& \frac{e^{\eta_{21}}}{e^{\eta_{21}}+e^{\eta_{22}}+e^{\eta_{23}}},\\
Z_{22} &=& \frac{e^{\eta_{22}}}{e^{\eta_{21}}+e^{\eta_{22}}+e^{\eta_{23}}},\\
Z_{23} &=& \frac{e^{\eta_{23}}}{e^{\eta_{21}}+e^{\eta_{22}}+e^{\eta_{23}}}.
\end{eqnarray*}
\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}
...
$$
p_c(x;\theta) = Z_{2c}, \quad c=1,2,3.
$$
Indeed, the softmax function guaranties that
$$
0 < Z_{2c} < 1, \quad Z_{21}+Z_{22}+Z_{23} = 1.
$$
...
$$
f(x;\theta) = \theta_{21}^{(0)} + \theta_{21}^{(1)} Z_{11} + \theta_{21}^{(2)} Z_{12}
$$
\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\end{itemize}
Above, $\eta$ controls the learning rate. Its choice is crucial. It varies from iterations, potentially adaptive, and can be a vector, i.e., one learning rate per weight (AdaGrad, RMSProp, Adam).
\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}
...
$$
This is a called {\bf stochastic gradient descent} (SGD).
\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}
\frametitle{SGD: batches and epochs}
...
\item For SGD, $S$ is called a {\bf batch}. The training set is divided in random, disjoint batches $S_1,\ldots,S_k$. The SGD is iterated on those batches. A pass through all the batches is called an {\bf epoch}.
\end{itemize}
\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}
