...
\vspace{0.3cm}
After the training, the tree contains final nodes. We use the notation below,
\begin{itemize}
...
For classification,
\begin{itemize}
\item The node $N(x)$ has a predicted probability vector: probabilities for each possible class
$$
...
The algorithm could go on splitting the feature space "forever" (entropy/impurity is zero). Several stopping rules can be used. For example in {\tt R} with {\tt rpart} (see {\tt ?rpart.control}):
\begin{itemize}
...
\item {\tt cp} (cost-complexity parameter): Any split that does not decrease the overall lack of fit by a factor of cp is not attempted (default to 0.1).
...
\vspace{0.3cm}
For trees, complexity is measured by the length of the tree: a shorter tree is simpler than a longer one.\\
\vspace{0.3cm}
...
\item Select the shortest tree among those that are equivalent to be the best one.
\end{itemize}
\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}
