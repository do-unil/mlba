...
\begin{frame}
\frametitle{Concept}
In summary, two correlated features can be replaced by one combination without loosing the information. \\
\vspace{0.3cm}
The resulting representation shows as much variance as possible: the information is kept.\\
\vspace{0.3cm}
...
\end{verbatim}
\normalsize
Together, PC1 and PC2 explain $95.8\%$ of the variation of the data. The scatter plot matrix shows that it is a good representation of the data with only two dimensions (PC$_1$, PC$_2$) : little separation is lost by projection.
\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}
\frametitle{PCA: example}
...
Correlation between the PC and the features $x$ can be computed to see how a PC is correlated to each feature.
\begin{center}
...
\item PC$_2$ is positively correlated with sepal width. The larger PC$_2$ the larger this feature.
...
\item The model is trained to recover $X$ at the end.
\end{itemize}
If, after training, the model can recover $X$ from $X$, then it can recover $X$ from its image in the intermediate layer $h(X)$. Thus, only $k$ dimensions would be needed to recover $X$. \\
\vspace{0.2cm}
Thus,
\begin{itemize}
