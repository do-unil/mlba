...
\end{itemize}
The objective of the intensive randomization is to build many small {\it uncorrelated} learners rather than one unique big learner.\\
\vspace{0.3cm}
...
Therefore, \\
\begin{itemize}
\item Points 1 and 3 are bagging.
...
$$
\item At each step $k$, the next model $f_{k+1}$ focuses on errors, potentially by giving more importance to the instance that are incorrectly classified by the current boosting model $F_k=\sum_{t=1}^k \alpha_t f_t$.
...
$$
\item Next model: $f_{k+1}$ minimizes $\sum_i 1_{\{y_i \neq f_{k+1}(x_i)\}} e^{-y_i F_{k}(x_i)}$.
\item Model weight:
$$
