...
# Video lecture
Below, you can find one of the lectures given by Ilia on web scraping and APIs during the [text mining course](https://hec-si.unil.ch/gide-api/web/syllabus/2840?base_url=https://www.unil.ch/hec/fr/home/menuinst/masters/management/cours-et-horaires.html?url=) of 2022.

<!-- {{< video https://drive.google.com/file/d/13hz208TzcE0WgnZiS48ZKafAEpnqp_cZ/preview >}} -->

<iframe width="780" height="500" src="https://drive.google.com/file/d/13hz208TzcE0WgnZiS48ZKafAEpnqp_cZ/preview" allow = "autoplay" allowfullscreen></iframe>

[In-class R script shown in the video above 📄](SeleniumRDemo.R.R)
...
**Note**: in the `xpath`, you must turn the quotes around `main` to simple quotes.
...
In this exercise, we practice the text extraction from a PDF file. First, we use the `pdf_text` function to read the text on the file "cs.pdf".
...
Note: the package `tabulizer` does not exist anymore. But we kept the details here. We still need to find a replacement for this package.

We tried to get it from its github page (<https://github.com/ropensci/tabulizer>), but I am note sure that this is working. Execute this code:
...
In this exercise, we use the OCR features of R to read a text from an image. First, we read the image using `image_read` in the package `magick`, the result is then passed to the `image_ocr` function in the package `tesseract`. 
...
In this exercise you are invited to create your API key for [The Guardian Open Plateform](https://open-platform.theguardian.com/) and to use it to extract papers using the `guardianapi` package.
...
From now on, you can tokenize and analyze the data. 
