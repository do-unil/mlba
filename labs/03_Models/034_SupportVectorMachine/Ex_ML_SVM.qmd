...
```{r py-best-scores, echo= F, include = F}
...
Note that the accuracy of `r round(best_score,2)*100`% is on the training set. The best parameters are `r best_grid_param`. We will use a new plotting library for seeing this evolution called `searborn`, which offers some great visualization tools.

Note that the accuracy of is on the training set. The best parameters are `r best_grid_param`. We will use a new plotting library for seeing this evolution called `searborn`, which offers some great visualization tools.
